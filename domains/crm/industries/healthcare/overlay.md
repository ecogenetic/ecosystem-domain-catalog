...
- **Patient** — a Contact receiving or seeking care, identified by a medical record number (MRN); may differ from the guarantor who pays.
- **ProviderOrganization** — an Account representing a practice, clinic, hospital, or referring organisation.
...
- **ConsentRecord** — the patient's explicit, scoped, revocable permission for communication and data sharing; every outreach checks it.
...
- Patient registeredWithProvider ProviderOrganization (many-to-one); the patient's home practice anchors the relationship.
